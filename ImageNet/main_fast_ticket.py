...
def parse_args():
    ...
    parser.add_argument('--eb_path', default=None, type=str)
...
def main():
    ...
    model = models.__dict__[configs.TRAIN.arch](pretrained=False)       # dense model

    # ################################## Load Ticket from Normal training ##################################
    if configs.eb_path:
        print('Load EB from {}'.format(configs.eb_path))
        checkpoint = torch.load(configs.eb_path, map_location='cpu')
        cfg_input = checkpoint['cfg']
        model = models.__dict__[configs.TRAIN.arch](pretrained=False, cfg=cfg_input)     # real pruned model
